...
db = databank.fromCSV("data/forecast_tunisia-2017Q2.csv");
...
level = Series();
level(endHist) = h.l_yf(endHist) - 0.1;
...
level = Series();
level(qq(2016,3)) = h.l_z(qq(2016,3));
...
% Create initial condition for the auxiliary variable for the persitent
% exchange rate shock
h.aux_l_s_long = 0 * h.l_s;
...
databank.toCSV(h, "csv/initial-conditions-2017Q2.csv", "decimals", 16);

save mat/dates-2017Q2.mat startHist endHist startFcast endFcast
